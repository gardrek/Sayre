...
function Mob.__index(table, key)
  ...
  if not value then
    value = rawget(ThisClass, key)
  end
  --[[if not value then
    error(
      'Attempt to access non-existant member ' .. key ..
      ' of class ' .. (rawget(ThisClass, 'class') or 'UnkownClass'),
      2
    )
  end]]
  return value
...
function Mob:new(template)
  return Mob.inherit(recursive_copy(ThisClass.template), template)
end
